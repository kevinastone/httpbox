...
pub fn stream_bytes(req: &mut Request) -> IronResult<Response> {
    ...
    let chunk_size = parse_query_value(
...
mod test {
    ...
    fn test_bytes() {
        ...
        let res = request::get(
            "http://localhost:3000/bytes/4?seed=1234",
            Headers::new(),
            &app,
        ).unwrap();
    ...
    fn test_stream_bytes() {
        ...
        let res = request::get(
            "http://localhost:3000/stream-bytes/4?seed=1234",
            Headers::new(),
            &app,
        ).unwrap();
    ...
    fn test_stream_bytes_with_chunk_size() {
        ...
        let res = request::get(
            "http://localhost:3000/stream-bytes/4?seed=1234&chunk_size=2",
            Headers::new(),
            &app,
        ).unwrap();
