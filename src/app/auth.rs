...
mod test {
    ...
    fn test_basic_no_authorization() {
        ...
        let res = request::get(
            "http://localhost:3000/basic-auth/my-username/my-password",
            Headers::new(),
            &app,
        ).unwrap();
    ...
    fn test_bearer_no_authorization() {
        ...
        let res = request::get(
            "http://localhost:3000/bearer-auth/my-token",
            Headers::new(),
            &app,
        ).unwrap();
    ...
    fn test_bearer_authorized() {
        let app = app();
        let mut headers = Headers::new();
        headers.set(headers::Authorization(headers::Bearer {
            token: "my-token".to_owned(),
        }));

        let res =
            request::get("http://localhost:3000/bearer-auth/my-token", headers, &app).unwrap();
